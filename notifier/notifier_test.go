...
//	"time"
//
//	"github.com/Financial-Times/annotations-publisher/external/draft"
...
//	publisher := NewAPI(draftAnnotationsClient, server.URL+"/notify", server.URL+"/__gtg", draft.testingClient, draft.testLog)
//
...
//	draftAnnotationsClient := &mockAnnotationsClient{}
//	publisher := NewAPI(draftAnnotationsClient, "/notify", "/__gtg", draft.testingClient, draft.testLog)
//
...
//	draftAnnotationsClient := &mockAnnotationsClient{}
//	publisher := NewAPI(draftAnnotationsClient, ":#", "/__gtg", draft.testingClient, draft.testLog)
//
//	body := make(map[string]interface{})
//	ctx := tid.TransactionAwareContext(context.Background(), "tid")
//	ctx = context.WithValue(ctx, CtxOriginSystemIDKey(OriginSystemIDHeader), "originSystemID")
...
//	draftAnnotationsClient := &mockAnnotationsClient{}
//	publisher := NewAPI(draftAnnotationsClient, "/publish", "/__gtg", draft.testingClient, draft.testLog)
//
//	body := make(map[string]interface{})
//	ctx := tid.TransactionAwareContext(context.Background(), "tid")
//	ctx = context.WithValue(ctx, CtxOriginSystemIDKey(OriginSystemIDHeader), "originSystemID")
...
//
//	draftAnnotationsClient := &mockAnnotationsClient{}
//	publisher := NewAPI(draftAnnotationsClient, "publishEndpoint", server.URL+"/__gtg", draft.testingClient, draft.testLog)
//	err := publisher.GTG()
//	assert.NoError(t, err)
//}
//
...
//	draftAnnotationsClient := &mockAnnotationsClient{}
//	publisher := NewAPI(draftAnnotationsClient, "publishEndpoint", "/__gtg", draft.testingClient, draft.testLog)
//	err := publisher.GTG()
...
//	draftAnnotationsClient := &mockAnnotationsClient{}
//	publisher := NewAPI(draftAnnotationsClient, "publishEndpoint", ":#", draft.testingClient, draft.testLog)
//	err := publisher.GTG()
//	assert.EqualError(t, err, "parse \":\": missing protocol scheme")
//}
//
...
//	defer server.Close()
//
//	publisher := NewAPI(draftAnnotationsClient, server.URL+"/notify", "http://www.example.com/__gtg", draft.testingClient, draft.testLog)
//
...
//	publisher := NewAPI(draftAnnotationsClient, "http://www.example.com/notify", "http://www.example.com/__gtg", draft.testingClient, draft.testLog)
//
//	ctx, cancel := context.WithTimeout(tid.TransactionAwareContext(context.Background(), "tid_test"), 50*time.Millisecond)
//	defer cancel()
//	err := publisher.PublishFromStore(ctx, uuid)
